    ...
    <main style="padding-top:10px;">
        <div class="row main-container">
            <div class="col-12 d-flex justify-content-center mb-2">
                <h2 class="text-center text-primary">Ac Current Graph</h2>
            </div>
            <div class="div-ddl">
                ...
                <div class="dropdown-container">
                    ...
                    <select id="device-select" v-model="selectedDeviceId" @change="fetchgraphdata" class="custom-select"
                        ...
                        <option v-for="device in deviceList.filter(device => device.deviceType === 'Ac Current')"
    ...
    methods: {
        async fetchgraphdata() {
            try {
                ...
                // Prepare data for all channels grouped by gearType
                const seriesData = [];

                channelData.forEach((channel) => {
                    const gearType = channel.gearType; // Access the gearType
                    const channelName = `${channel.channalId} - ${gearType}`; // Combine gearType and channelId for series name
                    const channelSeries = {
                        name: channelName,
