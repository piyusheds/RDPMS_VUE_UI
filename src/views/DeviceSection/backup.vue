                            ...
                            <li v-for="(device, index) in hut.devices" :key="index">
                                ...
                                <div class="device-details-content">
                                    ...
                                    <div v-if="device.devicetype === 'AC Voltage'" class="ac-voltage-content">
                                        <div v-for="n in 2" :key="'ac-voltage-' + n">
                                            ...
                                            <b-form-group :label="'Gear Type'" class="mb-3 mr-3"
                                                ...
                                                <b-form-select v-model="device['gearType' + n]" :options="gearOptions"
                                            ...
                                            <b-form-group label="Value Range" class="mb-3 ml-3 div_margin_left"
                                                ...
                                                <div class="d-flex align-items-center">
                                                    <b-form-input v-model="device['channelValueRange' + n + 'Min']"
                                                        placeholder="Min" :id="'ac-voltage-value-range-' + n + '-min'"
                                                        style="width: 40%; margin-right: 4%;"></b-form-input>
                                                    <span style="margin: 0 8px;">-</span>
                                                    <b-form-input v-model="device['channelValueRange' + n + 'Max']"
                                            ...
                                            <b-form-group label="Parameter Name:" class="mb-3 div_margin_left">
                                                <b-form-input v-model="device['channelsParameter' + n]"
                                    ...
                                    <div v-if="device.devicetype === 'AC Current'" class="ac-current-content">
                                        <div v-for="n in 6" :key="'ac-current-' + n">
                                            <b-form-group :label="'Channels'" class="mb-3 mr-3 padding_bottom">
                                                <b-form-group :label="'CH-' + n + ':'" class="mb-3 mr-3"></b-form-group>
                                            </b-form-group>
...
export default {
            ...
            huts: [
                {
                    ...
                    devices: [
                        {
                            devicetype: '', // e.g., 'AC Voltage', 'AC Current', 'DC Voltage', etc.
                            deviceid: '',
                            name: '',
                            detailsVisible: false,
                            gearType1: '', // for AC Voltage/AC Current/DC Voltage/DC Current
                            gearType2: '',
                            channelValueRange1Min: null, // Changed to null, can be a number later
                            channelValueRange1Max: null, // Changed to null, can be a number later
                            channelValueRange2Min: null,
                            channelValueRange2Max: null,
                            channelsParameter1: '',
                            channelsParameter2: '',
                            dcVoltageGears1: '',
                            dcVoltageGears2: '',
                            dcVoltageValueRange1Min: null,
                            dcVoltageValueRange1Max: null,
                            dcVoltageInput1: '',
                            dcVoltageInput2: '',
                            dcCurrentGears1: '',
                            dcCurrentGears2: '',
                            dcCurrentValueRange1Min: null, // Initializing as null (number)
                            dcCurrentValueRange1Max: null, // Initializing as null (number)
                            dcCurrentParam1: '',
                            dcCurrentParam2: '',
                            digitalInputGears1: '',
                            digitalInputGears2: '',
                            ...
                            digitalInputValue1: '',
                            digitalInputValue2: '',
                            // Add more if required for each device type
                        }
                    ]
                }
            ],
    ...
    methods: {
        ...
        //                             mappedDevice[`gearType${n}`] = device[`gearType${n}`] || ""; // Dynamically map gearType
        //                             mappedDevice[`channelValueRange${n}Min`] = device.channelValueRange1 || null; // Dynamically map valueRangeMin
        //                             mappedDevice[`channelValueRange${n}Max`] = device.channelValueRange2|| null; // Dynamically map valueRangeMax
        //                             mappedDevice[`channelsParameter${n}`] = device.channelsParameter || ""; // Dynamically map inputParam
        //                         }
        //                         break;
        //                     default:
        //                         break;
            ...
            try {
                ...
                if (response.data) {
                    ...
                    const mapDeviceAttributes = (device, tableName, channelCount) => {
                        ...
                        switch (tableName) {
                            case "Digital Input":
                                mappedDevice.parameter = device.parameterName || "";
                                break;
                            case "AC Voltage":
                            case "DC Voltage":
                            case "AC Current":
                            case "DC Current":
                                for (let n = 1; n <= channelCount; n++) {
                                    mappedDevice[`gearType${n}`] = device[`gearType`] || ""; // Dynamically map gearType
                                    mappedDevice[`channelValueRange${n}Min`] = device[`channelValueRange${n}`] || null; // Dynamically map valueRangeMin
                                    mappedDevice[`channelValueRange${n}Max`] = device[`channelValueRange${n}`] || null; // Dynamically map valueRangeMax
                                    mappedDevice[`channelsParameter${n}`] = device[`channelsParameter`] || ""; // Dynamically map inputParam
                                }
                                break;
                            default:
                    ...
                    // Function to fill the data into the hut
                    deviceTables.forEach((table) => {
                        if (table.data && Array.isArray(table.data)) {
                            table.data.forEach((device) => {
                                const deviceGroup = {
                                    ...
                                    devicetype: table.name,
                                    deviceid: device.deviceId,
                                    devices: [],  // This will hold the devices of this group
                                };
                                ...
                                const existingDeviceGroup = hut.devices.find(d => d.deviceid === device.deviceId);
                                if (existingDeviceGroup) {
                                    // Update the existing device by pushing the mapped device into its devices array
                                    existingDeviceGroup.devices.push(mappedDevice);
                                } else {
                                    // Add a new device group with the mapped device
                                    deviceGroup.devices.push(mappedDevice);
                                    hut.devices.push(deviceGroup);
                                }
                            });
